...
A real-time intelligent traffic control system that uses ultrasonic sensors to detect traffic density and AI logic to optimize traffic light duration.
...
│   │   ├── sensorController.js   # Handles sensor data & runs AI
│   │   └── decisionController.js # Helper for AI logic
...
3.  **Processing**:
    ...
    *   **Immediately** runs AI logic (`analyzeTrafficAndDecide`).
...
## 🧠 AI Logic
The system prioritizes the lane with the **shortest sensor distance**, assuming that cars are closer to the sensor (more traffic).
